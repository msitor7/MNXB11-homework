...
for
    log_file="wget.log"
    for url in "$@" ; do
	wget "$url" -0 "input_$counter" &>> "$log_file"
    ...
    if [ $? -ne 0 ]; then
	echo "Error: Failed to download $url. See $log_file for details".
	exit 1
    fi
    ...
    if file "input_$counter" grep -q "PDF document" ; then
        ...
        YOUR_CODE_HERE
          ...
          mv "input_$counter" "PDF/input_$counter"
            # else output message
            echo "File $CURRENTINPUTFILE is not a PDF, moving to directory notPDF..."
          mv "input_$counter" ·notPDF/input_$counter"
    else
